...
-   文体管理: トンマナ（Tone & Manner）を含む文体テンプレートの作成・管理
...
-   `/drafts` で保存一覧と編集・削除
-   `/writing-styles` で文体テンプレート管理（トンマナ設定含む）

## 文体テンプレート管理（/writing-styles）

文体テンプレートの作成・編集・管理を行います。以下の機能を提供します：

-   **文体プロパティ**: トーン、敬語レベル、文章スタイルなどの基本的な文体特性
-   **トンマナ（Tone & Manner）設定**:
    -   **ブランドボイスルール**: ブランド全体のトーンやボイスに関するガイドライン
    -   **すべきこと (Dos)**: 推奨される表現や書き方のリスト
    -   **すべきでないこと (Don'ts)**: 避けるべき表現や書き方のリスト
    -   **例文・フレーズ**: その文体を表す具体的なフレーズ例
-   **AI 文体分析**: サンプルテキストから文体の特徴を自動抽出
-   **Markdown エクスポート**: 文体テンプレートを Markdown 形式で出力

トンマナ設定により、ブランドの一貫性を保ちながら記事を生成できます。
