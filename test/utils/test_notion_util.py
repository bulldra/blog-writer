...
from app.notion_util import format_notion_page_for_context, get_default_notion_config
...
def test_format_notion_page_for_context():
    ...
    page_data = {
        "properties": {
            ...
            "created_date": {"type": "date", "date": {"start": "2024-01-01"}},
        },
...
def test_format_notion_page_minimal():
    ...
    # Minimal page data
    page_data: Dict[str, Any] = {"properties": {}}
...
def test_format_notion_page_with_rich_text():
    ...
    page_data = {
        "properties": {
            "title": {"title": [{"plain_text": "Rich Text Test"}]},
            "description": {
                "type": "rich_text",
                "rich_text": [
...
def test_format_notion_page_empty_properties():
    ...
    page_data = {
        "properties": {
            "title": {"title": [{"plain_text": "Filter Test"}]},
            "empty_select": {"type": "select", "select": None},
            "empty_rich_text": {"type": "rich_text", "rich_text": []},
            "valid_text": {
                "type": "rich_text",
