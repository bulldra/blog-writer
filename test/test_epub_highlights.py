...
from app.models import EpubHighlight, init_db
from app.routers.epub import (
    create_highlight, get_highlights, update_highlight, 
    delete_highlight, get_selected_highlights_for_context
)
...
def test_get_highlights_with_book_filter(mock_db):
    ...
    mock_query = Mock()
    mock_query.filter.return_value.order_by.return_value.all.return_value = [mock_highlight]
...
def test_update_highlight_success(mock_db):
    ...
    assert mock_highlight.selected_for_context == True
    mock_db.commit.assert_called_once()
...
def test_get_selected_highlights_for_context(mock_db):
    ...
    mock_db.query.return_value.filter.return_value.order_by.return_value.all.return_value = [
        mock_highlight1, mock_highlight2
    ]
...
def test_epub_highlight_model():
    ...
    assert highlight.selected_for_context == True