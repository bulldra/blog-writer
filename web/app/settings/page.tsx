...
export default function SettingsPage() {
	...
	// Directory picker modal states
	const [showPicker, setShowPicker] = useState<null | {
		target: 'obsidian' | 'epub'
	}>(null)
	...
	const openPicker = (target: 'obsidian' | 'epub') => {
		setShowPicker({ target })
	}

	const handlePick = (pickedPath: string) => {
		if (!showPicker) return
		if (showPicker.target === 'obsidian') {
			setObsidianDirInput(pickedPath)
		} else {
			setEpubDirInput(pickedPath)
		}
		setShowPicker(null)
	}
	...
	return (
		<main style={{ maxWidth: 720, margin: '2rem auto', padding: '0 1rem' }}>
			...
			<h2>Obsidian設定</h2>
			<p>
				Obsidianのハイライトディレクトリを設定することで、Kindle
				Highlightを利用できます。
			</p>
			...
			<section
				...
				<div
					...
					/>
					<button
						onClick={() => openPicker('obsidian')}
						style={{ padding: '8px 12px' }}>
						選択…
					</button>
					<button
						onClick={() =>
							saveObsidian(obsidianDirInput.trim() || '')
						}
			...
			<h2>EPUB設定</h2>
			<p>
				EPUBファイルが格納されているディレクトリを設定することで、書籍検索機能を利用できます。
			</p>
			...
			{showPicker && (
				<DirectoryPicker
					initialPath={
						showPicker.target === 'obsidian'
							? obsidianDirInput || undefined
							: epubDirInput || undefined
					}
					onSelect={handlePick}
