...
class NotionMCPSettings(BaseModel):
    ...
    enabled: bool = Field(default=False, description="Whether Notion integration is enabled")
    default_parent_id: str = Field(default="", description="Default parent page ID for new pages")
...
async def test_connection() -> Dict[str, Any]:
    ...
    try:
        ...
        if success:
            ...
            return {"connected": False, "error": "Failed to connect to Notion MCP server"}
...
async def list_pages(limit: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not settings.get("enabled", False):
            raise HTTPException(status_code=400, detail="Notion integration is disabled")
            
        if not settings.get("env", {}).get("NOTION_API_KEY"):
            raise HTTPException(status_code=400, detail="Notion API key is not configured")
